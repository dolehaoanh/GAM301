...
public class TurretController : MonoBehaviour
{
    ...
    // COMMENT OUT DE DUNG HAM MOI SU DUNG OBJECT POOLING CUA UNITY ENGINE
    // void Shoot()
    // {
    ...
    void Shoot()
    {
        // Thay thế Instantiate bằng việc lấy đạn từ ObjectPoolManager
        if (ObjectPoolManager.Instance != null)
        {
            ObjectPoolManager.Instance.GetBullet(firePoint.position, firePoint.rotation);
        }
        else
        {
            // Fallback phòng trường hợp quên kéo thả ObjectPoolManager trong Scene
            Instantiate(bulletPrefab, firePoint.position, firePoint.rotation);
        }
    }
